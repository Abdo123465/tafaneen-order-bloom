// src/pages/Kashakil80EnglishDisneyPage.tsx
...
const kashakil80EnglishDisneyItems = [
  {
    id: "kash80-eng-dis-1",
    name: "كشكول 80 ورقة انجليزي ديزني",
    price: 25,
    image: "/assets/kash80-eng-dis-princess.jpg",
    description: "كشكول انجليزي 80 ورقة، غلاف أميرات ديزني ملون، أو سيارات أو ميكي ماوس، ورق 60 جرام.",
    brand: "Disney",
    pages: 80,
    size: "22.5×16 سم",
    paperWeight: "60 جرام",
...
const Kashakil80EnglishDisneyPage = () => {
  const { addItem } = useCart();

  useEffect(() => {
    document.title = "كشاكيل 80 ورقة انجليزي ديزني | تفانين";
    const desc = "احصل على كشاكيل انجليزي 80 ورقة بتصاميم ديزني المميزة.";
      ...
      >
        <main className="container mx-auto px-4 py-10">
          ...
          <nav className="flex items-center gap-2 text-sm text-muted-foreground mb-8">
            ...
            <ArrowRight className="h-4 w-4" />
            <Link to="/notebooks/kashakil/80-pages/english" className="hover:text-primary">كراسة انجليزي</Link>
            <ArrowRight className="h-4 w-4" />
          ...
          <div className="text-center mb-12">
            ...
            />
            <h1 className="text-3xl lg:text-4xl font-bold mb-4">كشاكيل 80 ورقة انجليزي ديزني</h1>
            <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
              اختر كشكولك المفضل بتصميمات ديزني المحببة للأطفال
            </p>
          </div>

          {/* المنتجات بنفس الهيكل والستايل */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {kashakil80EnglishDisneyItems.map((item) => (
              <Card key={item.id} className="group hover:shadow-lg transition-all duration-300 hover:-translate-y-1 overflow-hidden">
                <CardContent className="p-0">
                  <div className="relative h-48 bg-gradient-to-br from-blue-100 to-indigo-100 flex items-center justify-center overflow-hidden">
                    ...
                    <div className="absolute bottom-3 left-3 bg-orange-600 text-white rounded-full px-3 py-1 text-xs font-bold">
                      {item.pages} ورقة
                    </div>
                  </div>
                  <div className="p-6">
          ...
          <div className="text-center mt-12">
            <Button asChild variant="outline">
              <Link to="/notebooks/kashakil/80-pages/english">العودة إلى كراسة انجليزي</Link>
            </Button>
          </div>
        </main>
      </div>
...
export default Kashakil80EnglishDisneyPage;
